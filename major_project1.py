...
# -------------------
# BASELINE 1: Simple CNN (no Siamese)
# -------------------
class SimpleCNN(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(6, 32, 3, padding=1),  # pre + post stacked
            nn.ReLU(),
            nn.MaxPool2d(2),
...
# -------------------
# BASELINE 2: Siamese CNN (NO transformer)
# -------------------
class SiameseCNN(nn.Module):
    ...
    def encode(self, x):
        f1, f2, f3 = self.backbone(x)
        p1 = self.pool(f1).view(x.size(0), -1)
        p2 = self.pool(f2).view(x.size(0), -1)
        p3 = self.pool(f3).view(x.size(0), -1)
...
# -------------------
# BASELINE 3: Transformer-only (no Siamese)
# -------------------
class TransformerOnly(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = SimpleBackbone()
        self.fusion = FusionTransformerBlock(128)

        self.pool = nn.AdaptiveAvgPool2d((1,1))
        self.fc = nn.Linear(128, 4)

    def forward(self, pre, post):
        _, _, f_pre = self.backbone(pre)
        _, _, f_post = self.backbone(post)

        f_pre, f_post = self.fusion(f_pre, f_post)

        diff = torch.abs(f_pre - f_post)
        pooled = self.pool(diff).view(diff.size(0), -1)
